  ...
  <div class="container">
    ...
    <div class="main-content">
      ...
      <div class="tweet-section">
        <div class="content-container" v-for="data in state?.filter">
          <div
            class="contents"
            v-for="datas in data?.tag_tweet"
            v-if="data?.tag_tweet != 0"
          >
            <div class="profile-pict">
              <img
                :src="'data:image/png;base64,' + datas?.tweets?.user?.profile"
                :alt="datas?.tweets?.user?.username"
                style="width: 50px; height: 50px; border-radius: 50%"
                v-if="datas?.tweets?.user?.profile != null"
              />

              <div class="no-profile" v-else>
                <span>
                  {{ datas?.tweets?.user?.username.charAt(0) }}
                </span>
              </div>
            </div>
            <div class="content">
              <div class="top-content">
                <div class="username-posted-on">
                  <h4>{{ datas?.tweets?.user?.username }}</h4>
                  <p style="text-transform: lowercase">
                    @{{ datas?.tweets?.user?.username }}
                  </p>
                  <p>{{ format_date(datas?.created_at) }}</p>
                </div>
                <div
                  class="edit"
                  v-if="datas?.tweets?.user?.email === state?.user?.email"
                >
                  <router-link :to="'/Edit-tweet/' + datas?.tweets?.id">
                  ...
                  <button
                    @click="deletePost(datas?.tweets?.id)"
                    style="display: none"
                    id="deletePost"
                  ></button>
                </div>
              ...
              <div class="tweet-text tweet-image">
                ...
                >
                  <p>{{ datas?.tweets?.tweet }}</p>
                  <p style="color: #0d6efd; cursor: pointer" v-if="datas != 0">
                    #{{ data?.name }}
                  </p>
                </div>
                <img
                  :src="'data:image/png;base64,' + datas?.tweets?.media"
                  alt="image"
                  width="450"
                  height="250"
                  v-if="datas?.tweets?.media != null"
                />
              </div>
              <div class="comments likes">
                <i class="fa-regular fa-heart"></i>
                <router-link :to="'/Comments/' + datas?.tweets?.id">
      ...
      <div class="comment-section">
        <div class="content-container" v-for="data in state?.filter">
          <div
            class="contents"
            v-for="datas in data?.tag_comment"
            v-if="data?.tag_comment != 0"
          >
            <div class="profile-pict">
              <img
                :src="'data:image/png;base64,' + datas?.comments?.user?.profile"
                :alt="datas?.comments?.user?.username"
                style="width: 50px; height: 50px; border-radius: 50%"
                v-if="datas?.comments?.user?.profile != null"
              />

              <div class="no-profile" v-else>
                <span>
                  {{ datas?.comments?.user?.username.charAt(0) }}
                </span>
              </div>
            </div>
            <div class="content">
              <div class="top-content">
                <div class="username-posted-on">
                  <h4>{{ datas?.comments?.user?.username }}</h4>
                  <p>@{{ datas?.comments?.user?.username }}</p>
                  <p>@{{ format_date(datas?.comments?.created_at) }}</p>
                </div>
                <div
                  class="edit"
                  v-if="state?.user?.email === datas?.comments?.user?.email"
                >
                  <router-link :to="'/Edit-comment/' + datas?.comment_id">
                    <i class="fa-regular fa-pen-to-square"></i>
                  </router-link>
                  ...
                  <button
                    id="deleteComment"
                    style="display: none"
                    @click="DeleteComment(datas?.comment_id)"
                  ></button>
                </div>
              </div>
              <div class="tweet-text tweet-image">
                <p>{{ datas?.comments?.comments }}</p>
                <p style="color: #0d6efd">#{{ data?.name }}</p>
              </div>
              <img
                :src="datas?.comments?.media"
                alt=""
                v-if="datas?.comments?.media != null"
              />
            </div>
          </div>
        </div>
    ...
    <div class="tags-search">
      ...
      <div class="tags">
        <h1>Tags</h1>
        <div class="tag" v-for="data in state?.filter">
...
export default {
  setup() {
    ...
    const getSearch = async () => {
      try {
        const { data } = await API.get(`main/search?query=${route.query.query}`)
    ...
    const Search = async (e) => {
      if (e.key === 'Enter') {
        try {
          const { data } = await API.get(`main/search?query=${FormData.query}`)
          state.filter = data.data
        } catch (err) {
